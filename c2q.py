...
image_datasets = {
    x: torch.utils.data.Subset(image_datasets_full[x], sub_indices[x])
    for x in ["train", "val"]
}
...
class DressedQuantumNet(nn.Module):
    ...
    def forward(self, input_features):
        ...
        for elem in q_in:
            q_out_elem = (
                torch.hstack(quantum_net(elem, self.q_params)).float().unsqueeze(0)
            )
...
def train_model(model, criterion, optimizer, scheduler, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ["train", "val"]:
            ...
            if phase == "val" and epoch_acc > best_acc:
                best_acc = epoch_acc
                best_model_wts = copy.deepcopy(model.state_dict())
            if phase == "valn" and epoch_loss < best_loss:
    ...
    print(
        "Best test loss: {:.4f} | Best test accuracy: {:.4f}".format(
            best_loss, best_acc
        )
    )
    return model
...
# %%
def visualize_model(model, num_images=12, fig_name="Predictions"):
    images_so_far = 0
    ...
    with torch.no_grad():
        ...
        for _i, (inputs, labels) in enumerate(dataloaders["val"]):
            ...
            for j in range(inputs.size()[0]):
                # ax[i//4][i%4].imshow(preds[[i]])
                print(preds[[j]], labels[[j]])
                if (preds[[j]] == 0 and labels[[j]] == 3) or (
                    preds[[j]] == 1 and labels[[j]] == 88
                ):
                    correct += 1
                else:
                    wrong += 1
                ...
                if images_so_far == num_images:
                    print("Correct images {}, Wrong images {}".format(correct, wrong))
                    return
